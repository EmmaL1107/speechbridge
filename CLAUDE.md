...
SpeechBridge is an intelligent speech semantic recovery system. It processes real-world complex speech input — handling pronunciation deviations, language barriers, and uncertain expressions — to recover user intent and output clear, accurate results. The pipeline: audio preprocessing → VAD → ASR (faster-whisper) → RAG glossary correction → LLM semantic recovery → output formatting.
...
The system follows a layered pipeline architecture orchestrated by `SpeechBridgePipeline` in `pipeline.py`:
...
Audio Input
  ...
  → ASR: whisper_engine.py (faster-whisper, large-v3-turbo)
  → [Optional] RAG Glossary: glossary.py (domain term correction)
  → Semantic Recovery: llm_engine.py (litellm — supports OpenAI/Anthropic/DeepSeek/Ollama)
...
1. **API mode** (`web/api.py`): Pure FastAPI REST API with WebSocket streaming. Endpoints: `POST /api/process`, `WS /api/stream`, `GET /api/scenes`, `GET /api/models`, `GET /api/health`
2. **Demo mode** (`web/demo.py`): Full-stack with Jinja2 templates (`templates/`), user auth (`auth.py` — HMAC-SHA256 token-based, cookie auth), task history, SQLite persistence via SQLAlchemy (`database.py`). Tables: `users`, `tasks`.

Global singleton pipeline is lazy-loaded in the web layer.
...
| `ASR_MODEL` | Whisper model: `tiny` / `base` / `small` / `medium` / `large-v3-turbo` |
